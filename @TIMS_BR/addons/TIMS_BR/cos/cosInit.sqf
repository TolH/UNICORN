...
COS_distance=450;//Set spawn distance
...
/*breakPatrol_FNC=compile preprocessFileLineNumbers "TIMS_BR\cos\patrolFnc.sqf";*/unitScript_FNC=compile preprocessFileLineNumbers "TIMS_BR\cos\addScript_Unit.sqf";vehScript_FNC=compile preprocessFileLineNumbers "TIMS_BR\cos\addScript_Vehicle.sqf";
...
_rad=40;// Radius increase increment for finding minimum spawn points
...
//_randomisation=2;
	if (_houses <= 10) 
		then {
	_civilians=2;// Civilians spawned
	_vehicles=0;// Moving Vehicles Spawned
	_parked=1;// Parked Vehicles Spawned
			};		
 	if (_houses <= 30 and _houses > 10) 
		then {
	_civilians=3;// Civilians spawned
	...
	_parked=1;// Parked Vehicles Spawned
 	...
 	if (_houses <= 70 and _houses > 30) 
		then {
	_civilians=6;// Civilians spawned
	_vehicles=1;// Moving Vehicles Spawned
	_parked=1;// Parked Vehicles Spawned
			};
...
// Create a trigger over town	
		...
		_trigAct=format ["null= [%1] execVM ""TIMS_BR\cos\cosCore.sqf"";server setvariable [%2,true];",str _foo,str _var];
...
//null=[] execVM "TIMS_BR\cos\localScript.sqf";// This shows messages for players during multiplayer
