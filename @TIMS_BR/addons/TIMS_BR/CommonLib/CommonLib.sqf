 ...
 * History:
 *   2011-01-25: Created.
 *   2011-11-19: Updated to v1.01 - New version of function dre_fnc_CL_PositionIsInsideMarker. Still backward compatible with v1.00.
 *   2011-12-17: Added debug methods and method ShowTitleTextAllClients.
 *   2011-12-23: Added method dre_fnc_CL_InitParams
 *   2013-09-21: Made Arma 3 compatible.
 *   2015-03-07: Added function dre_fnc_GetParamValue.
 */
...
dre_var_CL_SilentDebugMode = true;
 ...
 */
dre_fnc_CL_GetMarkerWithinRange = {
	...
	while {[_markerName] call dre_fnc_CL_MarkerExists} do {
...
dre_fnc_CL_GetRandomMarkerPos = {
    ...
    while {!_isInside} do {
        ...
        if ([[_rpx, _rpy, 0], _markerName] call dre_fnc_CL_PositionIsInsideMarker) then {
            _isInside = true;
        };
 ...
 * Updated in CommonLib v1.01 - Function now supports RECTANGLE and ELLIPSE at any angle, previously only RECTANGLEs with angle 0.
 */
dre_fnc_CL_PositionIsInsideMarker = {
...
dre_CL_ShowCommandTextEventArgs = []; // [unit, text]

"dre_CL_ShowCommandTextEventArgs" addPublicVariableEventHandler {
	dre_CL_ShowCommandTextEventArgs call dre_fnc_CL_ShowCommandTextLocal;
};
 ...
 *   _text: Text to show.
 */
dre_fnc_CL_ShowCommandTextLocal = {
    private ["_text"];
 ...
 *   _text: Text to show.
 */
dre_fnc_CL_ShowCommandTextAllClients = {
    private ["_text"];
    _text = _this select 0;
    
    dre_CL_ShowCommandTextEventArgs = [_text];
    publicVariable "dre_CL_ShowCommandTextEventArgs";
    dre_CL_ShowCommandTextEventArgs call dre_fnc_CL_ShowCommandTextLocal;
};

dre_CL_ShowTitleTextEventArgs = []; // [text]

"dre_CL_ShowTitleTextEventArgs" addPublicVariableEventHandler {
	dre_CL_ShowTitleTextEventArgs call dre_fnc_CL_ShowTitleTextLocal;
};
 ...
 *   [_showAsSubTitle]: Optional. true if the text are to be shown as a sub title. false if it's to be shown in the middle of the screen.
 */
dre_fnc_CL_ShowTitleTextAllClients = {
    private ["_text", "_showAsSubTitle"];
    _text = _this select 0;
    if (count _this > 1) then {_showAsSubTitle = _this select 1;} else {_showAsSubTitle = false};
    
    dre_CL_ShowTitleTextEventArgs = [_text, _showAsSubTitle];
    publicVariable "dre_CL_ShowTitleTextEventArgs";
    dre_CL_ShowTitleTextEventArgs call dre_fnc_CL_ShowTitleTextLocal;
};

dre_CL_AddScoreEventArgs = []; // [vehicleVarName, score]

"dre_CL_AddScoreEventArgs" addPublicVariableEventHandler {
    if (isServer) then {
        dre_CL_AddScoreEventArgs call dre_fnc_CL_AddScoreServer; 
    };
};

dre_fnc_CL_AddScoreServer = {
    dre_CL_AddScoreEventArgs call compile format ["%1 addScore %2;", dre_CL_AddScoreEventArgs select 0, dre_CL_AddScoreEventArgs select 1]; 
};
...
dre_fnc_CL_AddScore = {
    ...
    dre_CL_AddScoreEventArgs = [_vehicleVarName, _score];
    
    if (isServer) then {
        dre_CL_AddScoreEventArgs call dre_fnc_CL_AddScoreServer;
    }
    else {
        publicVariable "dre_CL_AddScoreEventArgs";
    };
};
 ...
 */
dre_fnc_CL_AddUnitsToGarbageCollector = {
    private ["_units"];
    _units = _this;
    
    if (isNil "dre_var_CL_GarbageCollectorUnits") then {
        dre_var_CL_GarbageCollectorUnits = [];
    };
    
    dre_var_CL_GarbageCollectorUnits = dre_var_CL_GarbageCollectorUnits + _units;
};

/*
 * Summary: Starts the garbage collector. Garbage collector removes empty groups and units added by function dre_fnc_CL_AddUnitsToGarbageCollector every 10 seconds.
 * Remarks:
 ...
 */
dre_fnc_CL_RunGarbageCollector = {
    ...
    if (count _this > 1) then {_cleanUpDistance = _this select 1;} else {_cleanUpDistance = 1000};
    if (count _this > 2) then { _debug = _this select 2; } else { _debug = false; };
    
    if (_debug) then {
        ["Garbage collector started."] call dre_fnc_CL_ShowDebugTextAllClients;
    };
    ...
    while {true} do {
        ...
        {
            if (_debug) then {
                ["Group '" + str _x + "' garbage collected."] call dre_fnc_CL_ShowDebugTextAllClients;
            };
        ...
        } foreach dre_var_CL_GarbageCollectorUnits;
        
        dre_var_CL_GarbageCollectorUnits = dre_var_CL_GarbageCollectorUnits - _unitsToDelete;
        
        {
            ...
            if (_debug) then {
                ["Unit '" + str _x + "' garbage collected."] call dre_fnc_CL_ShowDebugTextAllClients;
            };
            deleteVehicle _x;
            ...
            if (str _group != "<NULL-group>") then {
                if (count units _group == 0) then {
                    if (_debug) then {
                        ["Group '" + str _group + "' garbage collected."] call dre_fnc_CL_ShowDebugTextAllClients;
                    };
...
dre_CL_DebugTextEventArgs = []; // Empty
dre_CL_DebugMarkerEventArgs = []; // [name, position, size, direction, shape ("RECTANGLE" or "ELLIPSE"), markerColor, markerText (optional)] or alternatively [name, position, type, markerColor (optional), markerText (optional)]
dre_CL_DeleteDebugMarkerEventArgs = [];  // [name]

"dre_CL_DebugTextEventArgs" addPublicVariableEventHandler {
    dre_CL_DebugTextEventArgs call dre_fnc_CL_ShowDebugTextLocal;
};

"dre_CL_DebugMarkerEventArgs" addPublicVariableEventHandler {
    dre_CL_DebugMarkerEventArgs call dre_fnc_CL_SetDebugMarkerLocal;
};

"dre_CL_DeleteDebugMarkerEventArgs" addPublicVariableEventHandler {
    dre_CL_DeleteDebugMarkerEventArgs call dre_fnc_CL_DeleteDebugMarkerLocal;
};
 ...
 *   _text: Debug text.
 */
dre_fnc_CL_ShowDebugTextLocal = {
    private ["_minutes", "_seconds"];
    
    if (!isNull player) then {
        if (!dre_var_CL_SilentDebugMode) then {
 ...
 *   _text: Debug text.
 */
dre_fnc_CL_ShowDebugTextAllClients = {
    dre_CL_DebugTextEventArgs = _this;
    publicVariable "dre_CL_DebugTextEventArgs";
    dre_CL_DebugTextEventArgs call dre_fnc_CL_ShowDebugTextLocal;
};
 ...
 *   [_markerText]. Optional. Marker's text.
 */
dre_fnc_CL_SetDebugMarkerLocal = {
    ...
    if (!isNull player) then {
        if (!dre_var_CL_SilentDebugMode) then {
            ...
            // Delete old marker
            if ([_markerName] call dre_fnc_CL_MarkerExists) then {
 ...
 *   [_markerText]. Optional. Marker's text.
 */
dre_fnc_CL_SetDebugMarkerAllClients = {
    dre_CL_DebugMarkerEventArgs = _this;
    publicVariable "dre_CL_DebugMarkerEventArgs";
    dre_CL_DebugMarkerEventArgs call dre_fnc_CL_SetDebugMarkerLocal;
};
 ...
 */
dre_fnc_CL_DeleteDebugMarkerLocal = {
    private ["_markerName"];
    _markerName = _this select 0;
    deleteMarkerLocal _markerName;
};
 ...
 *   _markerName: Name of marker to delete.
 */
dre_fnc_CL_DeleteDebugMarkerAllClients = {
    dre_CL_DeleteDebugMarkerEventArgs = _this;
    publicVariable "dre_CL_DeleteDebugMarkerEventArgs";
    dre_CL_DeleteDebugMarkerEventArgs call dre_fnc_CL_DeleteDebugMarkerLocal;
};

dre_var_CL_CommonLibVersion = 3.0;
dre_var_commonLibInitialized = true;
